...
import (
	...
	"go-to-do-checklist/internal/http-server/handlers/help"
...
func main() {
	...
	router.Use(middleware2.JWTAuthMiddleware())

	router.Get("/help", help.Help())

	router.Get("/url", url.Get(log, storage))
	...
	router.With(middleware2.RequireAuth).Post("/url", url.New(log, storage))
	...
	router.Post("/login", users.Login(log, storage))
	router.Post("/refresh", users.Refresh(log, storage))
